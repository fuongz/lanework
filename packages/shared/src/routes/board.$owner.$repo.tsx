...
export function useBoardData() {
	...
	return useMemo(() => {
		...
		const filtered = board.cards.filter(
			(c) =>
				(!mine || c.assignees.includes(board.viewer)) &&
		...
		>
			...
			{!onCost ? (
				<div className="px-6 pt-5">
					...
					{/* Toolbar: view switcher + primary action, right-aligned (Height-style).
            One shared 36px control height. */}
					<div className="mt-4 flex items-center justify-end gap-2">
						<div
							role="tablist"
							aria-label="Board view"
							className="inline-flex h-9 items-center gap-1 rounded-xl border border-border/70 bg-background p-1 shadow-[0_1px_2px_rgba(0,0,0,0.04)]"
						>
							{TABS.map((t) => {
