...
function groupByColumn(
	cards: Cards,
	override: Record<string, ReviewColumn> = {},
) {
	...
	for (const card of cards)
		byColumn[override[card.path] ?? card.column].push(card);
...
function LocalBoard({
	...
	const moveCard = async (card: ReviewCard, to: ReviewColumn) => {
		...
		try {
			await setCardStatus({
				data: { owner, repo, path: card.path, status: to },
			});
		} catch (err) {
			setPending((p) => {
				const next = { ...p };
			...
			if (typeof window !== "undefined")
				window.alert(`Couldn't move card: ${msg}`);
		}
	};
	...
	return (
		...
		>
			...
			<DragOverlay dropAnimation={null}>
				{activeCard ? (
					<div className="w-76 rotate-2 opacity-90">
...
function DroppableColumn({
	...
	return (
		<motion.div
			...
			className={cn(
				"flex h-full w-[19rem] shrink-0 flex-col rounded-2xl px-1 ring-2 ring-transparent transition-colors",
				isOver && "bg-muted/60 ring-ring/20",
			)}
		>
				...
				{cards.length === 0 ? (
					...
					cards.map((card) => (
						<DraggableCard
							key={card.path}
							card={card}
							running={runningPaths.has(card.path)}
						/>
					))
				)}
			</div>
...
function DraggableCard({
	...
	return (
		<div
			...
			title={
				running
					? "An agent is working — open the card and Stop it to move this."
					: undefined
			}
